 ...
 */
export async function sendOutreachViaCallHippo(request, apiKey) {
    ...
    if (TEST_MOCK) {
        ...
        console.log(`[CallHippo][MOCK] Pretending to send ${preferredChannel} to ${target} for ${businessName}`);
        return {
            success: true,
    ...
    console.log(`[CallHippo] Outreach request for ${businessName} (${phoneNumber}) via ${preferredChannel}`);
    ...
    if (preferredChannel === "whatsapp") {
        try {
            ...
            console.warn(`[CallHippo] WhatsApp failed for ${phoneNumber}, falling back to SMS:`, whatsappError instanceof Error ? whatsappError.message : whatsappError);
        }
    }
    // Fallback to SMS
    try {
        ...
        if (FORCE_SUCCESS) {
            console.warn(`[CallHippo][FORCE_SUCCESS] Returning demo success for ${phoneNumber}`);
            return {
                success: true,
                channel: preferredChannel,
        ...
        if (FORCE_SUCCESS) {
            console.warn(`[CallHippo][FORCE_SUCCESS] API failed, returning demo success for ${phoneNumber}`);
            return {
                success: true,
                channel: preferredChannel,
                messageId: `forced-${Date.now()}`,
...
async function sendWhatsAppMessage(phoneNumber, message, apiKey) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            console.warn(`[CallHippo] WhatsApp API returned ${response.status}:`, errorText);
...
async function sendSmsMessage(phoneNumber, message, apiKey) {
    ...
    try {
        ...
        if (!response.ok) {
            const errorText = await response.text();
            console.warn(`[CallHippo] SMS API returned ${response.status}:`, errorText);
