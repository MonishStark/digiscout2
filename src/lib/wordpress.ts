...
function wrapHtmlBlock(content: string) {
	return `<!-- wp:html -->\n${content}\n<!-- /wp:html -->`;
}

function getSectionLayout(section: { layout?: string; variant?: string }) {
	return (section.layout || section.variant || "standard").toLowerCase();
}

function getSectionTitle(section: any, fallback: string) {
	return section?.headline || section?.title || fallback;
}

function renderStructuredHeroSection(schema: WebsiteSchema) {
	const hero = getSection(schema, "hero") as any;
	if (!hero) return "";
...
function renderStructuredFeaturesSection(schema: WebsiteSchema) {
	const features = getSection(schema, "features") as any;
	if (
		!features ||
		!Array.isArray(features.items) ||
		features.items.length === 0
...
function renderStructuredGallerySection(schema: WebsiteSchema) {
	const gallery = getSection(schema, "gallery") as any;
	if (!gallery || !Array.isArray(gallery.items) || gallery.items.length === 0) {
		return "";
	}
...
function renderStructuredTestimonialsSection(schema: WebsiteSchema) {
	const testimonials = getSection(schema, "testimonials") as any;
	if (
		!testimonials ||
		!Array.isArray(testimonials.items) ||
		testimonials.items.length === 0
...
function renderStructuredFaqSection(schema: WebsiteSchema) {
	const faq = getSection(schema, "faq") as any;
...
function renderStructuredContactSection(schema: WebsiteSchema) {
	const contact = getSection(schema, "contact") as any;
...
function renderStructuredCtaSection(schema: WebsiteSchema) {
	const cta = getSection(schema, "cta") as any;
	if (!cta) return "";
	const layout = getSectionLayout(cta);
	const title = getSectionTitle(cta, getSiteVoice(schema).ctaTitle);
	const body = cta.body || "";
	const buttonLabel =
		cta.buttonLabel || cta.primaryCta?.label || getSiteVoice(schema).ctaButton;
...
function renderStructuredSection(schema: WebsiteSchema, section: any) {
	switch (section?.type) {
		case "hero":
...
function buildHomePageBlocks(schema: WebsiteSchema) {
	return [
		...
		...(Array.isArray(schema.sections) ? schema.sections : []).map((section) =>
			renderStructuredSection(schema, section),
		),
	]
		.filter(Boolean)
		.join("\n\n");
