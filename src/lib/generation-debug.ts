...
export interface GenerationDebugSessionSummary {
	traceId: string;
	folderName: string;
	folderPath: string;
	createdAt: string;
...
export async function writeGenerationDebugFile(
	traceId: string,
	fileName: string,
	content: unknown,
	append = false,
	...
	try {
		await fetch(
			`${API_URL}/api/debug-generation/${encodeURIComponent(traceId)}/file`,
			{
				method: "POST",
				headers: {
...
export async function fetchGenerationDebugSummary(
	traceId: string,
): Promise<GenerationDebugSessionSummary | null> {
	if (!traceId) return null;
	try {
		const response = await fetch(
			`${API_URL}/api/debug-generation/${encodeURIComponent(traceId)}/summary`,
		);
		if (!response.ok) return null;
		return (await response.json()) as GenerationDebugSessionSummary;
	} catch (error) {
...
export function buildPreviewSummaryMarkdown(args: {
	traceId: string;
	schema: WebsiteSchema;
	renderedHtml: string;
	wordpressBlocks: string;
	summary?: GenerationDebugSessionSummary | null;
	debugFallbackUsed?: boolean;
}) {
	const sectionTypes = args.schema.sections.map((section) => section.type);
	const galleryCount = args.schema.sections.filter(
		(section) => section.type === "gallery",
	).length;
	const testimonialCount = args.schema.sections.filter(
		(section) => section.type === "testimonials",
	).length;
	const missingSections = ["hero", "contact"].filter(
		(type) =>
			!sectionTypes.includes(type as WebsiteSchema["sections"][number]["type"]),
	);
	...
	const fallbackUsed =
		args.debugFallbackUsed || Boolean(args.summary?.fallbackReason);
	...
	return [
		...
		parseRepairs.length
			? parseRepairs
					.map(
						(report) =>
							`- [${report.index}] ${report.originalType || "unknown"} -> ${report.finalType}${report.repaired.length ? ` | repaired: ${report.repaired.join(", ")}` : ""}${report.droppedFields.length ? ` | dropped: ${report.droppedFields.join(", ")}` : ""}`,
					)
					.join("\n")
			: "- None",
		"",
...
function getSectionLayout(section: WebsiteSchema["sections"][number]) {
	return ((section as any).layout || (section as any).variant || "standard")
		.toString()
		.toLowerCase();
}

function getRendererVariant(section: WebsiteSchema["sections"][number]) {
...
function getIgnoredSchemaFields(section: WebsiteSchema["sections"][number]) {
	...
	const allowed = new Set(
		allowedFieldsByType[section.type] || ["id", "type", "layout"],
	);
	return Object.keys(section).filter((key) => !allowed.has(key));
}

function isFallbackLayout(section: WebsiteSchema["sections"][number]) {
	const layout = getSectionLayout(section);
	switch (section.type) {
		case "hero":
			return ![
...
export function buildRendererVariantLog(args: {
	traceId: string;
	schema: WebsiteSchema;
	renderedHtml: string;
	wordpressBlocks: string;
...
export function renderWordPressDebugHtml(args: {
	schema: WebsiteSchema;
	wordpressBlocks: string;
