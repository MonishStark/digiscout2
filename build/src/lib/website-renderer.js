...
exports.renderWebsiteArtifact = renderWebsiteArtifact;
exports.createDefaultWebsiteArtifact = createDefaultWebsiteArtifact;
...
function renderHero(section, schema) {
    const layout = (section.layout || section.variant || "split")
        .toString()
        .toLowerCase();
    const image = section.media?.src || fallbackImageForCategory(schema.brand.category);
    const voice = getSiteVoice(schema);
    const stats = [
    ...
    const subheadline = section.subheadline &&
        !/premium|designed to convert|first impression|conversion-ready/i.test(section.subheadline)
        ? section.subheadline
        : voice.introLine;
...
function renderFeatures(schema, section) {
    const layout = (section.layout || "cards").toString().toLowerCase();
    const voice = getSiteVoice(schema);
...
function renderGallery(schema, section) {
    const layout = (section.layout || "asymmetrical")
        .toString()
        .toLowerCase();
    const voice = getSiteVoice(schema);
    const sectionTitle = section.title ||
...
function renderTestimonials(schema, section) {
    const layout = (section.layout || "floating-cards")
        .toString()
        .toLowerCase();
    const voice = getSiteVoice(schema);
    const sectionTitle = section.title ||
...
function renderCta(schema, section) {
    const voice = getSiteVoice(schema);
...
function renderFaq(schema, section) {
    const voice = getSiteVoice(schema);
...
<section class="site-section section-${escapeHtml(section.type)}">
  ...
  <div class="generic-list">
    ${Array.isArray(items) ? items.map((it) => `<div class="generic-item">${escapeHtml(it.title || it.name || it)}${it.description ? `<p>${escapeHtml(it.description)}</p>` : ""}</div>`).join("") : ""}
  </div>
</section>`;
    }
}
function renderPageBody(schema) {
    // Preserve Gemini-provided ordering. Only fall back to a fixed order when schema.sections is empty.
    const sectionsFromSchema = Array.isArray(schema.sections) && schema.sections.length
        ? schema.sections
        : null;
    const fallbackHero = {
        id: "hero-fallback",
    ...
    if (sectionsFromSchema) {
        ...
        if (!sectionsToRender.find((s) => s.type === "hero")) {
            sectionsToRender.unshift(fallbackHero);
        }
        // ensure contact exists at end
        if (!sectionsToRender.find((s) => s.type === "contact")) {
            sectionsToRender.push({ type: "contact" });
        }
    }
    else {
...
const buildCss = (schema) => {
    const theme = defaultTheme(schema.theme);
    const voice = getSiteVoice(schema);
    const shellWidth = theme.density === "compact"
        ? "1320px"
        : theme.density === "airy"
            ? "1180px"
            : "1240px";
    const bodyGrid = theme.accentMode === "earthy"
    ...
    const cardFill = theme.surfaceStyle === "solid"
        ? "var(--surface)"
        : theme.surfaceStyle === "outline"
            ? "transparent"
            : "color-mix(in srgb, var(--surface) 70%, transparent)";
    const sectionSpacingTop = theme.density === "compact"
        ? "56px"
        : theme.density === "airy"
            ? "120px"
            : "88px";
    const sectionSpacingBottom = theme.density === "compact"
        ? "56px"
        : theme.density === "airy"
            ? "120px"
            : "88px";
...
`;
function renderWebsiteArtifact(artifact) {
    const normalizedSchema = {
        ...
        seo: {
            title: artifact.schema.seo?.title ||
...
function createDefaultWebsiteArtifact(business, schema, content) {
    return {
        schema,
        html: content.html,
        css: content.css,
