...
exports.provisionLocalWordPressSite = provisionLocalWordPressSite;
exports.deleteLocalWordPressSite = deleteLocalWordPressSite;
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
const child_process_1 = require("child_process");
const laragon_apache_vhost_manager_1 = require("./laragon-apache-vhost-manager");
const laragon_hosts_file_manager_1 = require("./laragon-hosts-file-manager");
const laragon_mysql_manager_1 = require("./laragon-mysql-manager");
...
async function installWordPressCore(params) {
    const { siteSlug, siteUrl, adminUrl, blogName, blogDescription, adminUsername, adminPassword, adminEmail, step, addLog, timeoutMs = 30000, maxRetries = 3, } = params;
    ...
    let lastError = null;
    const cookieJar = {};
    function appendCookies(headers) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            addLog(step, "info", `WordPress installation attempt ${attempt}/${maxRetries} - initiating core install`);
            ...
            if (step1Response.status >= 400) {
                throw new Error(`Language selection step failed with status ${step1Response.status}`);
            }
            ...
            if (responseText.includes("Success") ||
                ...
                addLog(step, "info", `WordPress core installation completed successfully (HTTP ${step2Response.status})`);
                ...
                try {
                    ...
                    if (verifyResponse.status === 200 || verifyResponse.status === 302) {
                        addLog(step, "info", "WordPress admin interface verified and accessible");
                        return;
                    }
                }
                catch (verifyError) {
                    addLog(step, "warn", `Could not immediately verify wp-admin accessibility: ${verifyError instanceof Error ? verifyError.message : String(verifyError)}`);
                    // Still consider it success if install endpoint responded
                    return;
                }
            }
            if (responseText.includes("already installed")) {
                addLog(step, "info", "WordPress was already installed, skipping install step");
                return;
            }
            throw new Error(`Unexpected response (${step2Response.status}): ${responseText.substring(0, 200)}`);
        }
        catch (error) {
            lastError = error instanceof Error ? error : new Error(String(error));
            if (attempt < maxRetries) {
                const delayMs = Math.min(1000 * Math.pow(2, attempt - 1), 5000);
                addLog(step, "warn", `Installation attempt ${attempt} failed: ${lastError.message}. Retrying in ${delayMs}ms...`);
                await sleep(delayMs);
            }
        }
    }
    throw new Error(`WordPress installation failed after ${maxRetries} attempts: ${lastError?.message || "unknown error"}`);
}
async function waitForApacheEndpoints(params) {
    const { probeSiteUrl, probeAdminUrl, requireAdmin = false, step, addLog, timeoutMs = 30000, intervalMs = 1000, probeTimeoutMs = 3000, } = params;
    ...
    while (Date.now() < deadline) {
        ...
        if (siteProbe.ok && (!requireAdmin || adminProbe.ok)) {
            addLog(step, "info", `Apache endpoints verified: ${probeSiteUrl} (${siteProbe.status})${probeAdminUrl ? `, ${probeAdminUrl} (${adminProbe.status})` : ""}`);
            return;
        }
        ...
        if (attempt <= 3) {
            addLog(step, "info", `Checking endpoints (attempt ${attempt}): site=${siteState}; wp-admin=${adminState}`);
        }
        await sleep(intervalMs);
    }
    throw new Error(`Timeout: endpoints not ready after ${timeoutMs}ms. Site: ${probeSiteUrl}${probeAdminUrl ? `, Admin: ${probeAdminUrl}` : ""}`);
}
async function restartApacheStandalone(apacheExe, apacheDir) {
    try {
        (0, child_process_1.execSync)("taskkill /F /IM httpd.exe /T", { stdio: "pipe" });
    }
    catch {
        // Ignore if Apache was already stopped or had no matching process.
    }
    await sleep(1000);
    const apacheProcess = (0, child_process_1.spawn)(apacheExe, ["-w", "-d", apacheDir], {
        detached: true,
        stdio: "ignore",
        windowsHide: true,
    });
...
function resolveApacheRoot(apacheBinPath) {
    return path_1.default.dirname(apacheBinPath);
}
async function provisionLocalWordPressSite(params) {
    const logs = [];
    const addLog = (step, level, message) => {
    ...
    try {
        ...
        if (!templatePath || !sitesPath || !apacheConfPath || !apacheBinPath) {
            addLog("subsite_creation", "error", "Laragon configuration is incomplete");
            return {
                success: false,
                dryRun: false,
        ...
        const sitePath = path_1.default.join(sitesPath, siteSlug);
        addLog("subsite_creation", "info", `Creating WordPress site for: ${siteSlug}`);
        ...
        try {
            ...
            if (process.platform === "win32") {
                try {
                    (0, child_process_1.execSync)(`robocopy "${templatePath}" "${sitePath}" /E /DCOPY:DAT /COPY:DAT /PURGE`, {
                        stdio: "pipe",
                        encoding: "utf-8",
                    });
                    ...
                    addLog("subsite_creation", "info", `Robocopy returned an error, verifying copied content anyway: ${message}`);
                }
                let wpAdminExists = fs_1.default.existsSync(path_1.default.join(sitePath, "wp-admin"));
                let indexExists = fs_1.default.existsSync(path_1.default.join(sitePath, "index.php"));
                if (!wpAdminExists || !indexExists) {
                    addLog("subsite_creation", "info", "Robocopy did not produce expected WordPress files, falling back to manual copy");
                    copyDirRecursive(templatePath, sitePath);
                    wpAdminExists = fs_1.default.existsSync(path_1.default.join(sitePath, "wp-admin"));
                    indexExists = fs_1.default.existsSync(path_1.default.join(sitePath, "index.php"));
                }
            }
            else {
                copyDirRecursive(templatePath, sitePath);
            ...
            const wpAdminExists = fs_1.default.existsSync(path_1.default.join(sitePath, "wp-admin"));
            const indexExists = fs_1.default.existsSync(path_1.default.join(sitePath, "index.php"));
            const wpConfigExists = fs_1.default.existsSync(path_1.default.join(sitePath, "wp-config.php")) ||
                fs_1.default.existsSync(path_1.default.join(sitePath, "wp-config-sample.php"));
            addLog("subsite_creation", "info", `Template copy complete: ${copiedFileCount} files copied, wp-admin=${wpAdminExists}, index.php=${indexExists}, wp-config=${wpConfigExists}`);
            if (!wpAdminExists || !indexExists) {
                throw new Error("Template copy did not produce a valid WordPress install. Verify the template contents and permissions.");
            }
            addLog("subsite_creation", "info", `Site directory ready: ${sitePath}`);
        }
        catch (error) {
            addLog("subsite_creation", "error", `Failed to copy template: ${error instanceof Error ? error.message : String(error)}`);
            return {
                success: false,
                dryRun: false,
                provisioningStatus: "failed",
        ...
        try {
            const mysqlManager = new laragon_mysql_manager_1.MySQLDatabaseManager({
                host: mysqlHost,
                port: mysqlPort,
                user: dbUser,
            ...
            addLog("subsite_creation", "error", `Failed to create database: ${error instanceof Error ? error.message : String(error)}`);
            // Continue anyway - might already exist
        }
        // Step 3: Update wp-config.php
        addLog("subsite_creation", "info", "Updating wp-config.php");
        try {
            const wpConfigPath = path_1.default.join(sitePath, "wp-config.php");
            if (!fs_1.default.existsSync(wpConfigPath)) {
                throw new Error(`wp-config.php not found at ${wpConfigPath}`);
            }
            let wpConfig = fs_1.default.readFileSync(wpConfigPath, "utf-8");
            // Replace database credentials
            wpConfig = wpConfig.replace(/define\(\s*['"]DB_NAME['"]\s*,\s*['"][^'"]*['"]\s*\)/g, `define('DB_NAME', '${dbName}')`);
            wpConfig = wpConfig.replace(/define\(\s*['"]DB_USER['"]\s*,\s*['"][^'"]*['"]\s*\)/g, `define('DB_USER', '${dbUser}')`);
            wpConfig = wpConfig.replace(/define\(\s*['"]DB_PASSWORD['"]\s*,\s*['"][^'"]*['"]\s*\)/g, `define('DB_PASSWORD', '${dbPassword}')`);
            wpConfig = wpConfig.replace(/define\(\s*['"]DB_HOST['"]\s*,\s*['"][^'"]*['"]\s*\)/g, `define('DB_HOST', '${mysqlHost}:${mysqlPort}')`);
            fs_1.default.writeFileSync(wpConfigPath, wpConfig, "utf-8");
            addLog("subsite_creation", "info", "wp-config.php updated successfully");
        }
        catch (error) {
            addLog("subsite_creation", "error", `Failed to update wp-config.php: ${error instanceof Error ? error.message : String(error)}`);
            return {
                success: false,
                dryRun: false,
                provisioningStatus: "failed",
        ...
        try {
            const vhostManager = new laragon_apache_vhost_manager_1.ApacheVhostManager(apacheConfPath, apacheBinPath, localDomain);
            await vhostManager.createVhost({
                serverName: siteSlug,
                documentRoot: sitePath,
                port: 80,
            });
            addLog("subsite_creation", "info", `Apache vhost created for: ${hostName}`);
        }
        catch (error) {
            addLog("subsite_creation", "error", `Failed to create vhost: ${error instanceof Error ? error.message : String(error)}`);
            return {
                success: false,
                dryRun: false,
                provisioningStatus: "failed",
        ...
        try {
            const hostsManager = new laragon_hosts_file_manager_1.HostsFileManager();
            await hostsManager.addEntry(hostName, "127.0.0.1");
            addLog("subsite_creation", "info", `Hosts file entry added: 127.0.0.1 ${hostName}`);
        }
        catch (error) {
            const message = error instanceof Error ? error.message : String(error);
            if (/(permission denied|access is denied|eperm|eacces)/i.test(message)) {
                addLog("subsite_creation", "warn", `Could not update hosts file (${message}). Continuing with provisioning.`);
            }
            else {
                addLog("subsite_creation", "error", `Failed to add hosts entry: ${message}`);
                return {
                    success: false,
                    dryRun: false,
                    provisioningStatus: "failed",
        ...
        try {
            const apacheExe = path_1.default.join(apacheBinPath, "httpd.exe");
            const apacheDir = resolveApacheRoot(apacheBinPath);
            let apacheReloaded = false;
            try {
                (0, child_process_1.execSync)(`"${apacheExe}" -k graceful`, { stdio: "pipe" });
                apacheReloaded = true;
                addLog("subsite_creation", "info", "Apache reloaded successfully (graceful)");
            }
            catch (gracefulError) {
                addLog("subsite_creation", "info", `Graceful reload failed, attempting Apache restart: ${gracefulError instanceof Error ? gracefulError.message : String(gracefulError)}`);
            ...
            if (apacheReloaded) {
                try {
                    ...
                    addLog("subsite_creation", "warn", `Endpoints not ready after reload: ${verificationError instanceof Error ? verificationError.message : String(verificationError)}. Retrying...`);
                    await sleep(2000);
                    await waitForApacheEndpoints({
                        probeSiteUrl: siteUrl,
                        probeAdminUrl: adminUrl,
            ...
            const message = error instanceof Error ? error.message : String(error);
            addLog("subsite_creation", "error", `Apache reload/verification failed: ${message}`);
            return {
                success: false,
                dryRun: false,
                provisioningStatus: "failed",
        ...
        addLog("subsite_creation", "info", "Local WordPress site provisioning completed. Proxy URLs are available.");
        return {
            success: true,
            dryRun: false,
            message: "WordPress site files copied, wp-config created, and proxy URLs generated. Visit the proxy URLs to continue installation or manage the site.",
            site: {
                siteId: siteSlug,
                siteSlug,
                siteUrl,
        ...
        addLog("subsite_creation", "error", `Unexpected error: ${error instanceof Error ? error.message : String(error)}`);
        return {
            success: false,
            dryRun: false,
            provisioningStatus: "failed",
...
async function deleteLocalWordPressSite(siteSlug) {
    try {
        const sitesPath = process.env.LARAGON_SITES_PATH;
        const apacheConfPath = process.env.LARAGON_APACHE_CONF_PATH;
        const apacheBinPath = process.env.LARAGON_APACHE_BIN_PATH;
        ...
        try {
            if (fs_1.default.existsSync(sitePath)) {
                ...
                console.log(`[LocalWordPressProvisioner] Deleted site directory: ${sitePath}`);
            }
        }
        catch (error) {
        ...
        try {
            ...
            try {
                (0, child_process_1.execSync)(`"${apacheExe}" -k graceful`, { stdio: "pipe" });
                console.log(`[LocalWordPressProvisioner] Apache reloaded after vhost deletion`);
            }
            catch (e) {
                // Kill and restart if graceful fails
                try {
                    (0, child_process_1.execSync)("taskkill /F /IM httpd.exe", { stdio: "pipe" });
                    await restartApacheStandalone(apacheExe, apacheDir);
                    console.log(`[LocalWordPressProvisioner] Apache restarted after vhost deletion`);
                }
                catch (restartError) {
        ...
        try {
            ...
            console.log(`[LocalWordPressProvisioner] Removed hosts file entry: ${hostName}`);
        }
        catch (error) {
        ...
        throw new Error(`Failed to delete site: ${error instanceof Error ? error.message : String(error)}`);
    }
}
/**
