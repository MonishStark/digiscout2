...
import { sendOutreachViaCallHippo, } from "./src/lib/callhippo-service";
...
const generationDebugSessions = new Map();
...
function createGenerationTraceId(business) {
    ...
    const businessSlug = slugifyDebugSegment(business?.name || business?.businessName || business?.id || "site");
        ...
        try {
            ...
            if (restUrl && key) {
                ...
                const fetchResponse = await Promise.race([
                    ...
                    new Promise((_, reject) => setTimeout(() => reject(new Error(`Creative Direction timeout after ${model.timeoutMs}ms`)), model.timeoutMs)),
                ]);
                if (!fetchResponse.ok) {
                ...
                const response = await Promise.race([
                    ...
                    new Promise((_, reject) => setTimeout(() => reject(new Error(`Timeout after ${model.timeoutMs}ms`)), model.timeoutMs)),
                ]);
...
function parseLeadQualificationOutput(rawText) {
    ...
    try {
        ...
        return {
            ...
            websiteUri: typeof parsed.websiteUri === "string" ? parsed.websiteUri : undefined,
...
Return only valid JSON in this exact shape:
{
  ...
  "notes": "short explanation"
    ...
    return {
        ...
        phoneNumber: business.phoneNumber,
        confidence: "low",
        notes: lastError instanceof Error
            ? lastError.message
...
function parseWebsiteSchemaOutput(rawText, business, debugSession) {
    ...
    if (!candidateJson) {
        if (debugSession) {
            ...
            appendGenerationDebugError(debugSession, "parser_failure: no JSON object could be extracted");
        }
        return null;
    }
    try {
        ...
        if (!parsed || typeof parsed !== "object") {
            if (debugSession) {
                ...
                appendGenerationDebugError(debugSession, "parser_failure: parsed JSON was not an object");
            }
            return null;
        }
        const root = typeof parsed.schema === "object" && parsed.schema
            ? parsed.schema
            : parsed;
        const nestedSections = (Array.isArray(root.sections) && root.sections) ||
            (Array.isArray(parsed?.website?.sections) &&
                parsed.website.sections) ||
            (Array.isArray(parsed?.site?.sections) &&
                parsed.site.sections) ||
            null;
        ...
        function normalizeSectionShape(rawSections) {
            ...
            for (const [index, rawSection] of rawSections.entries()) {
                ...
                if (!section.type) {
                    const inferredType = inferSectionType(section.id) ||
                ...
                switch ((section.type || "").toLowerCase()) {
                    ...
                    case "gallery":
                        ...
                        if (!section.items.length) {
                            warnings.push(`gallery section ${index} normalized to empty items`);
                        }
                        break;
                    case "testimonials":
                ...
                if (debugSession && (repaired.length > 0 || droppedFields.length > 0)) {
                    debugSession.warnings.push(`section[${index}] ${report.originalType || "unknown"} -> ${report.finalType} (${repaired.join(", ") || "no repairs"})`);
                    appendGenerationDebugError(debugSession, `normalization_repair: ${JSON.stringify(report)}`);
                }
                if (Array.isArray(section.items) && section.items.length === 0) {
                    warnings.push(`section[${index}] ${section.type} has no items after normalization`);
                }
                reports.push(report);
        ...
        const normalizationResult = nestedSections && nestedSections.length > 0
        ...
        const merged = {
            ...
            theme: {
                ...
                typography: {
                    ...
                    heading: root.theme?.typography?.heading ||
                        root.theme?.typography?.headingFont ||
                        root.theme?.fontHeading ||
                        fallback.theme.typography.heading,
                    body: root.theme?.typography?.body ||
                        root.theme?.typography?.bodyFont ||
                        root.theme?.fontBody ||
                        fallback.theme.typography.body,
                },
                customCss: root.theme?.customCss ||
                    root?.customCss ||
                    fallback.theme?.customCss ||
                    "",
            },
            brand: {
                ...fallback.brand,
                ...(root.brand || {}),
                businessName: root.brand?.businessName ||
                    root?.businessName ||
                    fallback.brand.businessName,
                category: root.brand?.category ||
                    root?.category ||
            ...
            seo: {
                ...fallback.seo,
                ...(root.seo || {}),
                keywords: Array.isArray(root.seo?.keywords) && root.seo?.keywords.length > 0
                    ? root.seo.keywords
                    : fallback.seo.keywords,
            },
            sections: normalizationResult.sections && normalizationResult.sections.length > 0
        ...
        if (debugSession) {
            ...
            debugSession.malformedSections = normalizationResult.reports.filter((report) => report.repaired.length > 0 || report.droppedFields.length > 0);
            debugSession.sectionTypes = merged.sections.map((section) => section.type);
            debugSession.warnings.push(...normalizationResult.warnings);
            persistGenerationDebugFile(debugSession, "04-extracted-json.json", parsed);
        }
        return merged;
    }
    catch (error) {
        if (debugSession) {
            appendGenerationDebugError(debugSession, `parser_failure: ${error instanceof Error ? error.message : String(error)}`);
            persistGenerationDebugFile(debugSession, "04-extracted-json.json", {
...
function ensureNonTemplateCopy(schema, business) {
    const seed = hashSeed(`${business.id || business.name || "lead"}-${business.category || "category"}`);
    ...
    const pickVariant = (sectionType) => {
        if (sectionType === "hero") {
            if (categoryNorm.includes("salon") ||
                categoryNorm.includes("spa") ||
                categoryNorm.includes("wellness")) {
                return pickBySeed(["editorial-split", "magazine", "centered", "minimal", "split"], seed + 3);
            }
            if (categoryNorm.includes("gym") ||
        ...
        if (sectionType === "features") {
            if (categoryNorm.includes("dental") ||
                ...
                categoryNorm.includes("finance") ||
                categoryNorm.includes("consult")) {
                return pickBySeed(["editorial-list", "editorial-cards", "bento"], seed + 13);
            }
            if (categoryNorm.includes("salon") ||
                categoryNorm.includes("spa") ||
                categoryNorm.includes("wellness")) {
                return pickBySeed(["alternating-stack", "bento", "editorial-cards", "editorial-list"], seed + 15);
            }
            return pickBySeed(["bento", "editorial-cards", "editorial-list"], seed + 17);
        }
        if (sectionType === "gallery") {
            if (categoryNorm.includes("salon") ||
                categoryNorm.includes("spa") ||
                categoryNorm.includes("wellness")) {
                return pickBySeed(["stacked-collage", "editorial-mosaic", "collage"], seed + 19);
            }
            return pickBySeed(["editorial-mosaic", "stacked-collage"], seed + 19);
        }
        if (sectionType === "testimonials") {
            return pickBySeed(["floating-cards", "editorial-quotes", "spotlight"], seed + 23);
        }
        if (sectionType === "faq") {
    ...
    const nextSections = (schema.sections || []).map((section) => {
        ...
        if (!current || genericPattern.test(current)) {
            return {
                ...modified,
                subheadline: buildUniqueHeroSubheadline(businessName, categoryLabel, seed),
            };
        }
        return modified;
    });
...
function sanitizeThemeEnums(theme) {
    ...
    return {
        ...
        buttonStyle: sanitize(theme.buttonStyle, ["pill", "sharp", "ghost"], "pill"),
        surfaceStyle: sanitize(theme.surfaceStyle, ["glass", "solid", "outline"], "glass"),
        mediaShape: sanitize(theme.mediaShape, ["rounded", "arched", "portrait", "square"], "rounded"),
        density: sanitize(theme.density, ["airy", "balanced", "compact"], "balanced"),
        accentMode: sanitize(theme.accentMode, ["neon", "earthy", "luxury", "fresh"], "neon"),
    };
}
...
function buildCategorySpecificFeatures(category, businessName, seed) {
    ...
    if (categoryNorm.includes("real estate") ||
...
function buildCategorySpecificTestimonials(category, businessName, seed) {
    ...
    if (categoryNorm.includes("cafe") || categoryNorm.includes("restaurant")) {
        return [
            {
                quote: "The new site actually reflects what makes this place specialΓÇöit brought me back to visit.",
                author: name1,
                role: role1,
            },
            {
    ...
    if (categoryNorm.includes("salon") || categoryNorm.includes("spa")) {
        return [
            {
                quote: "The website shows professionalism and careΓÇöexactly what I experienced when I visited.",
                author: name1,
                role: role1,
            },
            {
    ...
    if (categoryNorm.includes("gym") || categoryNorm.includes("fitness")) {
        return [
            {
                quote: "The online tour showed real community energyΓÇöjoined immediately and haven't looked back.",
                author: name1,
                role: role1,
            },
            {
    ...
    if (categoryNorm.includes("real estate") ||
        ...
        return [
            {
                quote: "Their online listing brought clarity to a complex marketΓÇöguided me through the whole process with expertise.",
                author: name1,
                role: role1,
            },
            {
    ...
    if (categoryNorm.includes("dry clean") || categoryNorm.includes("laundry")) {
        return [
            {
                quote: "My premium items have never looked betterΓÇötrusted professionals who care about quality.",
                author: name1,
                role: role1,
            },
            {
    ...
    if (categoryNorm.includes("dry clean") || categoryNorm.includes("laundry")) {
        return [
            ...
            },
            {
                ...
                answer: "We stand behind our work and have industry insurance. We'll discuss solutions immediatelyΓÇöyour satisfaction matters.",
            },
        ];
    }
    return [
...
function createFallbackWebsiteSchema(business) {
    ...
    const buttonVariant = pickBySeed(["pill", "sharp", "ghost"], copySeed + 13);
    const mediaVariant = pickBySeed(["arched", "rounded", "portrait"], copySeed + 19);
    const densityVariant = pickBySeed(["airy", "balanced", "compact"], copySeed + 23);
    const accentVariant = pickBySeed(["earthy", "luxury", "fresh", "neon"], copySeed + 31);
    const categoryNorm = (business.category || "").toLowerCase();
    const heroVariant = categoryNorm.includes("salon") || categoryNorm.includes("spa")
        ? pickBySeed(["magazine", "editorial-split", "centered", "minimal", "split"], copySeed + 5)
        : layoutVariant === "minimal"
            ...
            : layoutVariant === "immersive"
                ...
                : layoutVariant === "split-screen"
                    ? "split"
                    : "split";
    const featureLayout = categoryNorm.includes("salon") || categoryNorm.includes("spa")
        ? pickBySeed(["bento", "alternating-stack", "editorial-cards"], copySeed + 9)
        : layoutVariant === "minimal"
            ? "list"
            : "cards";
    const heroImage = imagePool[0] ||
        "https://images.unsplash.com/photo-1497366754035-f200968a6e72?auto=format&fit=crop&w=1600&q=80";
    const heroSubheadline = buildUniqueHeroSubheadline(siteName, categoryLabel, copySeed);
    // Build category-specific features, testimonials, and faqs
    const features = buildCategorySpecificFeatures(business.category || "", siteName, copySeed);
    const testimonials = buildCategorySpecificTestimonials(business.category || "", siteName, copySeed);
    const faqs = buildCategorySpecificFaqs(business.category || "", siteName, copySeed);
    // Get section ordering variant
    const sectionOrder = buildSectionOrderPattern(business.category || "", copySeed);
    ...
    const schema = {
        ...
        theme: {
            name: design.name,
            style: design.style,
            radius: design.surfaceStyle === "solid"
                ? "20px"
                : layoutVariant === "minimal"
                    ? "16px"
                    : "28px",
...
app.post("/api/generate", async (req, res) => {
    try {
        ...
        logStderr(`[Generate] start traceId=${debugSession.traceId} business=${business.name} mode=${WEBSITE_GENERATION_MODE}`);
        res.setHeader("x-debug-generation-id", debugSession.traceId);
        res.setHeader("x-debug-generation-fallback", "false");
        persistGenerationDebugFile(debugSession, "01-business-input.json", buildBusinessDebugInput(business));
        // Check if TEST MODE is enabled
        if (WEBSITE_GENERATION_MODE === "template") {
            console.log("[Generate] Using TEMPLATE mode (TEST_MODE) - skipping Gemini");
            debugSession.fallbackReason = "template-mode";
            appendGenerationDebugError(debugSession, "fallback_triggered: template mode");
            res.setHeader("x-debug-generation-fallback", "true");
            const fallbackSchema = createFallbackWebsiteSchema(business);
            persistGenerationDebugFile(debugSession, "05-normalized-schema.json", fallbackSchema);
            return res.json(fallbackSchema);
        }
        // Normal Gemini flow
        if (!GENAI_KEY && !process.env.GEMINI_REST_URL) {
            debugSession.fallbackReason = "missing-config";
            appendGenerationDebugError(debugSession, "fallback_triggered: no Gemini API configuration found");
            res.setHeader("x-debug-generation-fallback", "true");
            const fallbackSchema = createFallbackWebsiteSchema(business);
            persistGenerationDebugFile(debugSession, "05-normalized-schema.json", fallbackSchema);
            return res.json(fallbackSchema);
        ...
        persistGenerationDebugFile(debugSession, "01a-creative-direction.json", creativeDirection);
        const buildImageBlock = (b) => {
            const sources = collectBusinessImages(b);
            return sources.length
                ? sources
                    .map((u, i) => `${i + 1}. ${u}${i < (b.photos || []).length ? " (business photo / Google Maps source)" : " (additional reference image)"}`)
                    .join("\n")
        ...
        const buildReviewsBlock = (b) => {
            if (Array.isArray(b.reviews) && b.reviews.length) {
                return b.reviews
                    .map((r, i) => `${i + 1}. ${r.rating || ""} - ${r.text || r.comment || ""}`)
                    .join("\n");
            }
...
Return only valid JSON matching the WebsiteSchema TypeScript interface. Make sure the returned theme contains the "designDNA" object exactly as described.`;
                ...
                try {
                    ...
                    if (restUrl && GENAI_KEY) {
                        ...
                        const fetchResponse = await Promise.race([
                            ...
                            new Promise((_, reject) => setTimeout(() => reject(new Error(`REST timeout after ${model.timeoutMs}ms`)), model.timeoutMs)),
                        ]);
                        if (!fetchResponse.ok) {
                            throw new Error(`REST failed (${fetchResponse.status}): ${await fetchResponse.text()}`);
                        }
                        ...
                        const response = (await Promise.race([
                            ...
                            new Promise((_, reject) => setTimeout(() => reject(new Error(`${model.name} request timed out after ${model.timeoutMs}ms`)), model.timeoutMs)),
                        ]));
                        const result = await response.response;
        ...
        persistGenerationDebugFile(debugSession, "03-gemini-raw-response.txt", rawText);
        const parsedSchema = parseWebsiteSchemaOutput(rawText, business, debugSession);
        if (!parsedSchema) {
            console.warn("[Generate] Gemini output could not be parsed as WebsiteSchema, using fallback schema.");
            debugSession.fallbackReason = "parse-failure";
            appendGenerationDebugError(debugSession, "fallback_triggered: parse failure");
            res.setHeader("x-debug-generation-fallback", "true");
            const fallbackSchema = createFallbackWebsiteSchema(business);
            persistGenerationDebugFile(debugSession, "05-normalized-schema.json", fallbackSchema);
            return res.json(fallbackSchema);
        ...
        const sectionSummary = (finalSchema.sections || []).map((section, index) => ({
            index,
            type: section.type,
        ...
        logStderr(`[Generate] layouts traceId=${debugSession.traceId} sections=${JSON.stringify(sectionSummary)} repairs=${JSON.stringify(validation.repairs || [])}`);
        ...
        try {
            ...
            if (premiumHtml) {
                ...
                persistGenerationDebugFile(debugSession, "05c-wordpress-html-final.html", premiumHtml);
            }
        }
        catch (wordpressHtmlError) {
            finalSchema._renderSource = "local-builder";
            appendGenerationDebugError(debugSession, `wordpress_html_generation_failed: ${wordpressHtmlError instanceof Error ? wordpressHtmlError.message : String(wordpressHtmlError)}`);
        }
        ...
        const schemaHash = crypto
            ...
            .digest("hex");
        logStderr(`[Generate] complete traceId=${debugSession.traceId} business=${business.name} renderSource=${renderSource} wpHtml=${wpHtml ? `yes(${wpHtml.length})` : "no"} schemaSha1=${schemaHash}`);
        // 4. Trace Log to DB
        try {
        ...
        persistGenerationDebugFile(debugSession, "05-normalized-schema.json", finalSchema);
        debugSession.sectionTypes = finalSchema.sections.map((section) => section.type);
        res.setHeader("x-debug-generation-fallback", "false");
        return res.json(finalSchema);
    }
    catch (error) {
        const fallbackSchema = createFallbackWebsiteSchema(req.body);
        const debugSession = req.body && req.body.name
            ? Array.from(generationDebugSessions.values()).find((session) => session.businessName === req.body.name)
            : undefined;
        if (debugSession) {
            appendGenerationDebugError(debugSession, `route_error: ${error instanceof Error ? error.message : String(error)}`);
            persistGenerationDebugFile(debugSession, "05-normalized-schema.json", fallbackSchema);
            res.setHeader("x-debug-generation-id", debugSession.traceId);
            res.setHeader("x-debug-generation-fallback", "true");
        }
...
app.post("/api/debug-generation/:traceId/file", (req, res) => {
    ...
    persistGenerationDebugFile(session, fileName, content ?? "", Boolean(append));
    return res.json({ success: true, traceId, fileName });
});
app.get("/api/debug-generation/:traceId/summary", (req, res) => {
    const { traceId } = req.params;
    const session = getGenerationDebugSession(traceId);
    if (!session) {
        return res.status(404).json({ error: "Unknown debug generation trace" });
...
app.post("/api/deploy", async (req, res) => {
    try {
        ...
        const siteName = `${businessName
            .toLowerCase()
            .replace(/[^a-z0-9]+/g, "-")
            .replace(/^-+|-+$/g, "") || "digital-scout"}-${Date.now()}`;
...
app.post("/api/enrich-business", async (req, res) => {
    try {
        ...
        function categoryImageSuggestions(cat, name) {
...
app.post("/api/qualify-leads", async (req, res) => {
    try {
        const { businesses, city } = req.body;
        if (!Array.isArray(businesses)) {
            return res.status(400).json({ error: "Missing businesses array" });
        }
        const candidates = businesses.filter((business) => business && typeof business.name === "string");
        const qualifications = await runWithConcurrency(candidates, 3, async (business) => {
        ...
        const qualifiedBusinesses = qualifications
            .filter(({ qualification }) => !qualification.hasWebsite &&
...
app.post("/api/wordpress/provision-site", async (req, res) => {
    try {
        ...
        logStderr(`[Provisioning] queue request projectId=${projectId} business=${business.name} traceId=${websiteSchema.meta?.traceId || "n/a"} renderSource=${renderSource} wpHtml=${wpHtml ? `yes(${wpHtml.length})` : "no"}`);
        const jobId = crypto.randomUUID();
        const traceId = websiteSchema.meta?.traceId ||
        ...
        const previewExpiresAt = isPreview
            ...
            : null;
        const [existing] = await pool.query(`SELECT id FROM provisioning_jobs WHERE project_id = ? LIMIT 1`, [projectId]);
        const targetStatus = status || "pending";
        let activeJobId = jobId;
        if (existing && existing.length > 0) {
            activeJobId = existing[0].id;
            await pool.query(`UPDATE provisioning_jobs SET website_schema = ?, status = ?, trace_id = ?, updated_at = NOW() WHERE project_id = ?`, [JSON.stringify(websiteSchema), targetStatus, traceId, projectId]);
        }
        else {
            await pool.query(`INSERT INTO provisioning_jobs (id, project_id, business_name, website_schema, status, trace_id, is_preview, preview_expires_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [
                jobId,
                projectId,
                business.name,
...
app.get("/api/wordpress/site-status/:projectId", async (req, res) => {
    ...
    try {
        const [rows] = await pool.query(`SELECT status, logs, subdomain, subdomain_url, wp_admin_url, ssl_status, wp_admin_user, wp_admin_pass_encrypted 
			 FROM provisioning_jobs 
			 LEFT JOIN isolated_deployments ON provisioning_jobs.project_id = isolated_deployments.project_id
			 WHERE provisioning_jobs.project_id = ? ORDER BY provisioning_jobs.created_at DESC LIMIT 1`, [projectId]);
        if (!rows || rows.length === 0) {
            return res.status(404).json({ error: "Job not found" });
        }
        const rootDomain = process.env.WP_ROOT_DOMAIN || "digiscout.online";
        const liveUrl = rows[0].subdomain_url ||
            (rows[0].subdomain ? `http://${rows[0].subdomain}.${rootDomain}` : null);
        const adminUrl = rows[0].wp_admin_url ||
            (rows[0].subdomain
                ? `http://${rows[0].subdomain}.${rootDomain}/wp-admin`
                : null);
        const effectiveStatus = rows[0].status === "completed" || liveUrl || adminUrl
            ? "completed"
            : rows[0].status;
...
app.post("/api/outreach/send", async (req, res) => {
    try {
        ...
        if (result.success) {
            console.log(`[Outreach] Successfully sent via ${result.channel} to ${phoneNumber}`);
            return res.json({
                success: true,
            ...
            console.warn(`[Outreach] Failed to send to ${phoneNumber}: ${result.error}`);
            return res.status(500).json({
                success: false,
...
app.get("/api/business-ai-chat/:leadId", async (req, res) => {
    try {
        ...
        const [messages] = await pool.query("SELECT role, content, created_at FROM lead_ai_messages WHERE lead_id = ? ORDER BY id ASC", [leadId]);
...
app.post("/api/business-ai-chat", async (req, res) => {
    try {
        ...
        if (latestMessage && latestMessage.role === "user") {
            await pool.query("INSERT INTO lead_ai_messages (lead_id, conversation_id, role, content) VALUES (?, ?, ?, ?)", [leadId, conversationId || leadId, "user", latestMessage.content]);
        }
        // 2. Fetch full history for local conversational memory
        const [dbHistory] = await pool.query("SELECT role, content FROM lead_ai_messages WHERE lead_id = ? ORDER BY id ASC", [leadId]);
        const chatContents = dbHistory.map((m) => ({
        ...
        const reviewsText = Array.isArray(businessContext.reviews) && businessContext.reviews.length
            ? businessContext.reviews.map((r, i) => `${i + 1}. [Rating: ${r.rating || "N/A"}] "${r.text || r.comment || ""}"`).join("\n")
...
3. Be highly structured and readable. Format your answers in professional Markdown with bullet points, bold opportunities, and clean comparison tables. Keep paragraphs strategic and concise.`;
        ...
        if (fullResponseText.trim()) {
            await pool.query("INSERT INTO lead_ai_messages (lead_id, conversation_id, role, content) VALUES (?, ?, ?, ?)", [leadId, conversationId || leadId, "model", fullResponseText]);
        }
        res.end();
    }
    catch (error) {
...
async function pollSslStatus() {
    try {
        const [deployments] = await pool.query(`SELECT * FROM isolated_deployments WHERE ssl_status = 'pending' LIMIT 5`);
        for (const dep of deployments) {
            ...
            try {
                ...
                await pool.query(`UPDATE isolated_deployments SET ssl_status = 'valid', subdomain_url = ?, wp_admin_url = ? WHERE id = ?`, [httpsUrl, `${httpsUrl}/wp-admin`, dep.id]);
            }
            catch (error) {
                // SSL not ready yet
...
async function pollCleanupPreviewSites() {
    try {
        const [deployments] = await pool.query(`SELECT project_id, preview_expires_at, status FROM provisioning_jobs WHERE preview_expires_at < NOW() AND status != 'cleaned' LIMIT 10`);
        for (const dep of deployments) {
            ...
            try {
                ...
                await pool.query(`UPDATE provisioning_jobs SET status = 'cleaned' WHERE project_id = ?`, [dep.project_id]);
